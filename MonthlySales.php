...
$query = "select product.ProductName AS 'Product Name', productinfo.BuyPrice AS 'Cost per item (Rm)', SUM(purchasedetail.noItem) AS 'Total sold', productinfo.SellPrice-productinfo.BuyPrice 
          AS 'Profit per Item (Rm)', (productinfo.SellPrice-productinfo.BuyPrice)*SUM(purchasedetail.noItem) AS 'Total income (Rm)', SUM(purchasedetail.noItem)*productinfo.SellPrice 
          AS 'Total revenue (Rm)' from product join productinfo join purchase join purchasedetail where product.ProductID=purchasedetail.ProductID AND 
          purchasedetail.Purchase_D_ID=purchase.Purchase_D_ID AND product.ProductID=productinfo.ProductID AND Month(purchase.PurchaseDate)= '$month' 
          AND Year(purchase.PurchaseDate) = '$year' GROUP BY purchasedetail.ProductID;";  
...
<nav style="justify-content: space-between;align-items:center">
    ...
    <div class="header">
      ...
      <a href="#" class=""  ><h1><b>KoopVentory : Monthly Sales</b></h1></a>
    </div>
...
<br>
<form>
    <h3>Monthly Sales for <?php echo $month; echo "-"; echo $year ?></h3>
...
<h4>&nbsp &nbsp &nbsp &nbsp &nbsp &nbsp Total Monthly income : Rm <?php echo $income ?></h4>
<h3>&nbsp &nbsp &nbsp &nbsp &nbsp Monthly Revenue</h3>
