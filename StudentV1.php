...
$UserID = $_GET['UserID'];
$mysqli_page = $db->query("SELECT substring_index(substring_index(Details, ';', 1), ':', -1) AS 'FullName', substring_index(substring_index(Details, ';', 2), ':', -1) AS 'gender', 
                substring_index(substring_index(Details, ';', 3), ':', -1) AS 'email', substring_index(substring_index(Details, ';', 4), ':', -1) AS 'Phone', 
                substring_index(substring_index(Details, ';', 5), ':', -1) AS 'IC', DateTime from editlog where UserID= '$UserID' And DateTime IN (SELECT max(DateTime) FROM editlog);");
...
if(isset($_POST["Restore"])){
    ...
    $get_banner="update studentinfo set gender=?, email=?, PhoneNum=?, IC=? where UserID=?";
...
<div class="header-right">
  <br>
  <a class="active" href="editStudent.php?UserID=<?php echo $UserID; ?>">Back</a>
</div>
