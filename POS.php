...
foreach($tab_result as $key=>$value){
	...
	$sub_row = $db_handle->runQuery("SELECT product.ProductName,  probar.ProductCode, productinfo.*, stock.quantity FROM product JOIN productinfo join probar join stock 
              WHERE productinfo.Categoryname = '".$tab_result[$key]["Categoryname"]."' AND product.ProductID=productinfo.ProductID AND product.ProductID=stock.ProductID AND product.ProductID=probar.ProductID;");
	foreach($sub_row as $key=>$value)
 	{
  	$tab_content .= '
  	<div class="col-md-3" style="margin-bottom:36px;">
  	<form method="post" action="POS.php?action=add&id='.$sub_row[$key]["ProductID"].' ">
    ...
    if($quan>$st){
      echo '<script>alert ("Not enough stock for this quantity");window.location.href = "POS.php";</script>';
		  ...
		  if(!empty($_POST["quantity"])) {
			  $productByCode = $db_handle->runQuery("SELECT product.ProductName, probar.ProductCode, productinfo.SellPrice FROM product join productinfo join probar WHERE probar.ProductCode='" . $_POST["code"] . "' AND 
                          product.ProductID=productinfo.ProductID AND product.ProductID=probar.ProductID");
...
if(isset($_POST['add'])){
    $productByCode = $db_handle->runQuery("SELECT product.ProductName, probar.ProductCode, productinfo.SellPrice FROM product join productinfo join probar WHERE product.ProductCode='" . $_POST["bar"] . "' 
                      AND product.ProductID=productinfo.ProductID AND product.ProductID=probar.ProductID");
...
<nav style="justify-content: space-between;align-items:center">
    <div class="header" style="background-image: url('images/tds.jpg');">
        <br>
...
<a id="btnEmpty" href="POS.php?action=empty">Empty Cart </a>
<?php
...
?>	
<br>
<form role="form" method="post" action="pos_transac.php">
    ...
    foreach ($_SESSION["cart_item"] as $item){
		...
		?>
          ...
          <?php echo "Rm ". number_format($item_price,2); ?>
				...
				<td style="text-align:center;"><a href="POS.php?action=remove&code=<?php echo $item["code"]; ?>" class="btnRemoveAction"><img src="icon-delete.png" alt="Remove Item" /></a></td>
				</tr>
				<?php
