...
$query = "select product.ProductName AS 'Product Name', productinfo.BuyPrice AS 'Cost per item (Rm)', SUM(purchasedetail.noItem) AS 'Total sold', productinfo.SellPrice-productinfo.BuyPrice 
          ...
          AS 'Total revenue (Rm)' from product join productinfo join purchase join purchasedetail where product.ProductID=productinfo.ProductID AND purchasedetail.Purchase_D_ID=purchase.Purchase_D_ID 
          AND purchasedetail.ProductID=product.ProductID AND purchase.PurchaseDate= '$date' GROUP BY purchasedetail.ProductID;";  
...
<table style="width:30%;text-align:center">
    ...
    <?php
    $mysqli_page = $db->query("select product.ProductName, stock.quantity from product join stock join productinfo where product.ProductID=stock.ProductID and stock.quantity<=productinfo.lowLevel 
                    AND product.ProductID=productinfo.ProductID;");
    $id;
